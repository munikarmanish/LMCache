...
async def process_single_prompt(
    ...
    async with semaphore:  # Acquire semaphore to limit concurrent requests
        # write_resp(f"Sending prompt {prompt_index + 1}/{total_prompts}\n")
...
async def main(args):
    ...
    # warmup_df.to_csv("/tmp/warmup_round.csv", index=False)
    # benchmark_df.to_csv("/tmp/query_round.csv", index=False)

    # Print results
    warmup_mean_ttft = trimmed_mean(
        warmup_df.query("successful == True")["ttft"], args.trim_fraction
    )
    warmup_p99_ttft = warmup_df.query("successful == True")["ttft"].quantile(0.99)
    query_mean_ttft = trimmed_mean(
        benchmark_df.query("successful == True")["ttft"], args.trim_fraction
    )
    query_p99_ttft = benchmark_df.query("successful == True")["ttft"].quantile(0.99)
    ...
    print(f"Warmup round p99 TTFT: {warmup_p99_ttft:.3f}s")
    ...
    print(f"Query round p99 TTFT: {query_p99_ttft:.3f}s")
